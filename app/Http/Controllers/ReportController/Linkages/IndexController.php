...
class IndexController extends Controller
{
    public function index(Request $request){
        ...
        // Determine the date range based on quarter or half
        if ($quarter) {
            switch ($quarter) {
                case 'Q1':
        ...
        $linkages = Linkages::whereBetween('created_at', [$startDate, $endDate])->get();

        $pdf = PDF::loadView('admin.reports.linkages.linkages', compact('linkages', 'year', 'quarter', 'half'));
        ...
        $fileName = 'LINKAGES_' . $request->input('year') . $period . '.pdf';
        ...
        // return $pdf->stream('LINKAGES.pdf');
    }
}
